...
const RoomDetails : React.FC<DetailsAreaProps> = ({
    ...
    const [usersInput, setUserInput] = useState<string>('');
    const userService = new UserService();
    useEffect(() => {
        const fetchUsers = async ()
    }, [usersInput])
    console.log(isdropdownUsersOpen)
    return (
        <>
            {
                !channelInfo ? null :
                <div className="detail-area shrink-0 border-l-[1px] border-gray-700 ml-auto flex flex-col overflow-auto">
                        ...
                        <div className={` w-auto h-auto ${isdropdownUsersOpen ? 'block' : 'hidden'}`}>
                                    <Dropdown label="" dismissOnClick={false} className={`w-28`}  placement="bottom" renderTrigger={() =>
